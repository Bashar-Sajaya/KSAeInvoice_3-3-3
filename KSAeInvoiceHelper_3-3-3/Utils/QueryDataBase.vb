...
FROM            (SELECT TaxID,myID, UnitCode,ItemName,  InvoiceQty,  PriceAmount * ExchangePrice AS PriceAmount , PriceAmount * InvoiceQty * ExchangePrice AS TotalPriceAmount , TotalRowDiscount  * ExchangePrice  As TotalRowDiscount , (PriceAmount * InvoiceQty * ExchangePrice ) - ( TotalRowDiscount  * ExchangePrice) As TotalPriceAmountAfterDiscount 
...
FROM     (SELECT TaxID,myID, UnitCode, InvoiceQty,TotalRowDiscount, LineExtensionAmount, TaxAmount, ((PriceAmount * InvoiceQty * ExchangePrice ) - ( TotalRowDiscount  * ExchangePrice)) + TaxAmount AS RoundingAmount, TaxCategoryPercent, ItemName, PriceAmount, AlowanceChargeAmount, ExchangePrice, 
                                    TaxExemption,SourceFiscalYearID,SourceVoucherTypeID,SourceVoucherNo, SourceStr
                  FROM      (SELECT TaxID,myID, UnitCode, InvoiceQty,ISNULL(DiscountValue, 0) As TotalRowDiscount , ISNULL(InvoiceQty, 0) * ISNULL(PriceAmount, 0) - (ISNULL(DiscountValue, 0) + ISNULL(headerDiscount, 0) * ISNULL(ItemPerc, 0)) AS LineExtensionAmount, (ISNULL(InvoiceQty, 0) 
                                     ...
                                     FROM      (SELECT StrVoucherDetails_2.TaxID ,StrVoucherDetails_2.RowNo AS myID, ISNULL(ISNULL(StrUnits.UnitCodeE, StrUnits.UnitCodeA), 'PCE') AS UnitCode, StrVoucherDetails_2.Qty AS InvoiceQty, StrVoucherDetails_2.ItemDesc AS ItemName, 
                                    ...
                                    AS QDetailsInfo
                  UNION ALL
                  SELECT TaxID,myID, UnitCode, InvoiceQty, TotalRowDiscount ,LineExtensionAmount, TaxAmount, LineExtensionAmount + TaxAmount AS RoundingAmount, TaxCategoryPercent, ItemName, PriceAmount, AlowanceChargeAmount, ExchangePrice, 
                                    TaxExemption,0 as SourceFiscalYearID ,0 as 	SourceVoucherTypeID,0 as SourceVoucherNo, SourceStr
                  FROM     (SELECT TaxID,myID, UnitCode, InvoiceQty,ISNULL(DiscountValue, 0) As TotalRowDiscount, ISNULL(InvoiceQty, 0) * ISNULL(PriceAmount, 0) - (ISNULL(DiscountValue, 0) + ISNULL(headerDiscount, 0) * ISNULL(ItemPerc, 0)) AS LineExtensionAmount, IsTaxable * (ISNULL(InvoiceQty, 0) 
                                    ...
                                    FROM      (SELECT QTaxData.TaxID ,RecInvoiceDetails_2.RowNo AS myID, 'PCE' AS UnitCode, 1 AS InvoiceQty, ISNULL(ISNULL(SysSaleTypes.TypeNameE, SysSaleTypes.TypeNameA), ISNULL(GLChartAcc.ChartAccNameE, 
...
FROM            SysTaxTypes AS SysTaxTypes_1 RIGHT OUTER JOIN
                  ...
                  FROM     (SELECT TaxID,myID, UnitCode, InvoiceQty,ISNULL(DiscountValue, 0) As TotalRowDiscount, ISNULL(InvoiceQty, 0) * ISNULL(PriceAmount, 0) - (ISNULL(DiscountValue, 0) + ISNULL(headerDiscount, 0) * ISNULL(ItemPerc, 0)) AS LineExtensionAmount, IsTaxable * (ISNULL(InvoiceQty, 0) 
                                                      * ISNULL(PriceAmount, 0) - (ISNULL(DiscountValue, 0) + ISNULL(headerDiscount, 0) * ISNULL(ItemPerc, 0))) * ISNULL(TaxPerc, 0) AS TaxAmount, IsTaxable * ISNULL(TaxPerc, 0) * 100 AS TaxCategoryPercent, ItemName, 
                                                      PriceAmount, ISNULL(DiscountValue, 0) + ISNULL(headerDiscount, 0) * ISNULL(ItemPerc, 0) AS AlowanceChargeAmount, ExchangePrice, TaxExemption,SourceStr
                                    FROM      (SELECT  SysTaxTypes_3.TaxID,AstVoucherDetails_2.RowNo AS myID, ISNULL(ISNULL(StrUnits.UnitCodeE, StrUnits.UnitCodeA), 'PCE') AS UnitCode, 1 AS InvoiceQty, ISNULL(AstAssets.AssetNameE, AstAssets.AssetNameA) AS ItemName, 
